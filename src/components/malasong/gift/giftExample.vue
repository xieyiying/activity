<template>
    <!-- <div class="gift">
        <div class="News_logo">            
            <img src="../total_img/getAward.png" class="giftimg">
            <img src="../total_img/startgetReward.png" class="clickimg" :class="[rotate?'fa fa-arrow-down go':'fa fa-arrow-down aa']" @click="start">            
        </div>
						...
						<div :class="{'wheel-bg6':true}">
							<div class="prize-list">
								<ul class="ulOne" :class="{'win':indexa===0&&prize_list.length==3}">
									<li v-for="(item,index) in arrOne" :class="{'win':index===indexa}" :style="{ zIndex: item.zIndex,transform: item.deg }" :key="index"></li>
								</ul>
								<ul class="ulTwo" :class="{'win':indexa===arrOne.length-1}">
									<li v-for="(item,index) in arrTwo" :class="{'win':index===indexb}" :style="{ zIndex: item.zIndex,transform: item.deg }" :key="index"></li>
...
import { hexMD5 } from '../../../router/Md5.js';
    ...
    //     },
    //     beforeCreate(){
    //         document.getElementById('titleId').innerHTML = '点击抽奖';    
    //     },
    //     mounted(){  
    //     },
    //     methods:{           
